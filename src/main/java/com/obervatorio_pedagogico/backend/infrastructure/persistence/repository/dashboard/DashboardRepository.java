...
public interface DashboardRepository extends JpaRepository<Disciplina, Long> {
    ...
    "where nota.tipo = :tipoNota and nota.ordem = :ordem and disciplina.periodo_letivo = :periodo and not frequencia_situacao.situacao_disciplina in ('REPROVADO_POR_FALTA', 'CANCELADO', 'TRANCADO') GROUP BY nota.id", nativeQuery = true)
    ...
    @Query(value = "select sum(nota.valor) / count(1) from t_nota nota " +
    "join t_disciplina disciplina on disciplina.id = nota.id_disciplina " +
    "where nota.tipo = :tipoNota and nota.ordem = :ordem and disciplina.periodo_letivo = :periodo GROUP BY nota.id", nativeQuery = true)
    ...
    @Query(value = "select sum(nota.valor) / count(1) from t_nota nota " +
    "join t_disciplina disciplina on disciplina.id = nota.id_disciplina " +
    "where nota.tipo = :tipoNota and disciplina.periodo_letivo = :periodo GROUP BY nota.id", nativeQuery = true)
    public Float obterMediaDeNotaPorTipoPeriodo(@Param("tipoNota") String tipoNota, @Param("periodo") String periodo);

    @Query(value = "select sum(nota.valor) / count(1) from t_nota nota " +
    "join t_disciplina disciplina on disciplina.id = nota.id_disciplina " +
    "join t_frequencia_situacao frequencia_situacao on frequencia_situacao.id_disciplina = disciplina.id " +
    "where nota.tipo = :tipoNota and not frequencia_situacao.situacao_disciplina in ('REPROVADO_POR_FALTA', 'CANCELADO', 'TRANCADO') and disciplina.periodo_letivo = :periodo GROUP BY nota.id", nativeQuery = true)
    ...
    "where disciplina.periodo_letivo = :periodo GROUP BY frequencia_situacao.id", nativeQuery = true)
    ...
    "join t_disciplina disciplina on disciplina.id = nota.id_disciplina " +
    "where nota.tipo = 'NOTA' and disciplina.periodo_letivo = :periodo GROUP BY nota.id", nativeQuery = true)
    public Float obterMenorNotaPorPeriodo(@Param("periodo") String periodo);

    @Query(value = "select min(nota.valor) from t_nota nota " +
    "join t_disciplina disciplina on disciplina.id = nota.id_disciplina " +
    "join t_frequencia_situacao frequencia_situacao on frequencia_situacao.id_disciplina = disciplina.id " +
    "where not frequencia_situacao.situacao_disciplina in ('REPROVADO_POR_FALTA', 'CANCELADO', 'TRANCADO') and nota.tipo = 'NOTA' and disciplina.periodo_letivo = :periodo GROUP BY nota.id", nativeQuery = true)
